...
| 目录 | 内容 |
|------|------|
| [A-Python基础](A-Python基础/) | **A1** Pythonic 与标准库；**A2** FastAPI 深度；**A3** 并发与异步；**A4** 测试与 CI（`A4-测试与CI/` 的 `demos/` 内建议建 `.venv` 再 `pytest`）。各章均为 `理论讲解.md` + `demos/`（FastAPI / 并发 / 测试三章在 **A2–A4**）。 |
| [B-后端工程化](B-后端工程化/) | **B1** 数据库深度；**B2** 网络与 HTTP 基础（`B2-网络与HTTP基础/`，体例与 A1 对齐：分节小结与对照实验）；**B3** Docker 与部署；**B4** 消息队列与任务队列；**B5** SSE 与 WebSocket；**B6** 微服务与系统设计；**B7** 设计模式。各章 `理论讲解.md` + 可运行 `demos/`（以各章 `demos/README.md` 为准）；**B7** 含 **GoF 23 整合速览（§2）** |
